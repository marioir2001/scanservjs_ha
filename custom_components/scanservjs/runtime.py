...
class ScanservJSRuntime:
    ...
    async def async_scan(self, device_id: str, profile: dict[str, Any]) -> dict[str, Any]:
        ...
        async with self._scan_lock:
            ...
            try:
                ...
                action = str(profile.get(CONF_FILE_ACTION) or "").strip()

                if bool(profile.get(CONF_SPLIT_PDF, False)):
                    await self.client.async_run_file_action(
                        filename, SPLIT_PDF_ACTION
                    )
                    executed_actions.append(SPLIT_PDF_ACTION)

                    split_filenames = await self._async_wait_for_split_files(
                        filename
                    )
                    file_info["split_files"] = split_filenames
                    file_info["split_count"] = len(split_filenames)
                ...
                if executed_actions:
                    file_info["actions"] = executed_actions
                    # Keep the old attribute for dashboard/backwards compatibility.
                    file_info["action"] = executed_actions[-1]
    ...
    async def _async_wait_for_split_files(
        self, original_filename: str
    ) -> list[str]:
        """Wait until the numbered PDFs created by split_pdf are visible."""
        original = PurePath(original_filename)
        stem = original.stem
        suffix = original.suffix

        if not suffix:
            raise ScanservJSApiError(
                "Die zu teilende Datei hat keine Dateiendung."
            )

        pattern = re.compile(
            rf"^{re.escape(stem)}_\d{{3}}{re.escape(suffix)}$",
            re.IGNORECASE,
        )

        # ScanservJS usually finishes the action before replying. The retry loop
        # also covers delayed filesystem visibility on slower systems.
        for attempt in range(21):
            files = await self.client.async_list_files()
            split_filenames = sorted(
                {
                    name
                    for item in files
                    if not bool(item.get("isDirectory", False))
        ...
        raise ScanservJSApiError(
            "Die von split_pdf erzeugten Einzeldateien wurden nicht in "
            "der ScanservJS-Dateiliste gefunden."
        )
